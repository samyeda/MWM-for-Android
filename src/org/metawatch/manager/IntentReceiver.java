...
public class IntentReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		MetaWatch.logDebug("intent: " +intent);


		Bundle b = intent.getExtras();
		for (String key : b.keySet()) {
			MetaWatch.logDebug("extras: " + key);
		}
		//MetaWatch.logDebug("datastring: " +intent.getDataString());
		...
		if (action.equals("android.intent.action.PROVIDER_CHANGED")) {
			...
			if (!Utils.isGmailAccessSupported(context)) {
				...
				MetaWatch.logDebug("count for " + recipient + ": " + bundle.getInt("count"));
