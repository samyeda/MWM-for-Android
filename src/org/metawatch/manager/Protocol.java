...
public class Protocol {
	...
	public static final byte Msg_GetDeviceType = 0x01;
	public static final byte Msg_GetDeviceTypeResponse = 0x02;
	public static final byte Msg_GetInfoString = 0x03;
	public static final byte Msg_GetInfoStringResponse = 0x04;
	public static final byte Msg_DiagnosticLoopback = 0x05;

	public static final byte Msg_SetVibrateMode = 0x23;
	public static final byte Msg_SetRTC = 0x26;

	public static final byte Msg_NValCommand = 0x30;
	public static final byte Msg_NValResponse = 0x31;
	public static final byte Msg_StatusChangeEvent = 0x33;
	public static final byte Msg_ButtonEventMessage = 0x34;

	public static final byte Msg_WriteBuffer = 0x40;
	public static final byte Msg_ConfigureMode = 0x41;
	public static final byte Msg_ConfigIdleBufferSize = 0x42;
	public static final byte Msg_UpdateDisplay = 0x43;
	public static final byte Msg_LoadTemplate = 0x44;
	public static final byte Msg_EnableButton = 0x46;
	public static final byte Msg_DisableButton = 0x47;


	public static final byte REPLAY = 30;
	public static final byte COPY_MODE_SELECT = 0x10;
	...
	public static void processSendQueue() {
		...
		Thread thread = new Thread() {
			public void run() {
				MetaWatch.logVerbose("send queue start");
				...
				MetaWatch.logVerbose("send queue finished");
				isSending = false;
			}
		};
		thread.start();
	...
	public static void send(byte[] bytes) throws IOException, NullPointerException {
		...
		MetaWatch.logVerbose("sending " +toProtocolString(bytes));

		MetaWatchService.outputStream.write(bytes);
		MetaWatchService.outputStream.write(crc(bytes));
	...
	public static void sendRtcNow(Context context) {
		try {
			...
			bytes[2] = Protocol.Msg_SetRTC; // set rtc
	...
	public static void loadTemplate(int mode) {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_LoadTemplate;
		bytes[3] = (byte) mode;
	...
	public static void activateBuffer(int mode) {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_ConfigIdleBufferSize; // activate buffer
		bytes[3] = (byte) mode;
	...
	public static void updateDisplay(int bufferType) {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_UpdateDisplay;
		bytes[3] = (byte) (bufferType | COPY_MODE_SELECT);
	...
	public static void vibrate(int on, int off, int cycles) {
		...
		bytes[0] = 0x01;
		bytes[1] = 12; // delka
		bytes[2] = Protocol.Msg_SetVibrateMode; // set vibrate
		bytes[3] = 0x00; // unused
	...
	public static void writeBuffer() {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_WriteBuffer; // write lcd buffer
	...
	public static void enableButton(int button, int type, int code) {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_EnableButton; // enable button
		bytes[3] = 0; // not used
	...
	public static void disableButton(int button, int type) {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_DisableButton; // disable button
		bytes[3] = 0; // not used
	...
	public static void configureMode() {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_ConfigureMode;
		bytes[3] = 0;
	...
	public static void getDeviceType() {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_GetDeviceType;
		bytes[3] = 0;
	...
	public static void queryNvalTime() {
		...
		bytes[0] = 0x01;
		bytes[1] = (byte) (bytes.length+2); // length
		bytes[2] = Protocol.Msg_NValCommand; // nval operations
		bytes[3] = 0x01; // read
	...
	public static void sendOledBufferPart(byte[] display, int start, int length, boolean startScroll, boolean last) {
		...
		MetaWatch.logDebug("sending oled buffer part, start: " + start + ", length: " + length);
	...
	public static String MsgToName(byte msgid) {

		switch(msgid)
		{
		case Msg_GetDeviceType :         return "GetDevType";
		case Msg_GetDeviceTypeResponse : return "GetDevTypeResp";
		case Msg_GetInfoString :         return "GetInfoString";
		case Msg_GetInfoStringResponse :   return "GetInfoTypeResp";

		case Msg_SetVibrateMode :        return "SetVibrateMode";
		case Msg_SetRTC :                return "SetRTC";

		case Msg_NValCommand :           return "NValCommand";
		case Msg_NValResponse :          return "NValResponse";
		case Msg_StatusChangeEvent :     return "StatusChangeEvent";
		case Msg_ButtonEventMessage :    return "ButtonEventMessage";

		case Msg_WriteBuffer :           return "WriteBuffer";
		case Msg_ConfigureMode :         return "ConfigureMode";
		case Msg_ConfigIdleBufferSize :  return "ConfigIdleBufSize";
		case Msg_UpdateDisplay :         return "UpdateDisplay";
		case Msg_LoadTemplate :          return "LoadTemplate";

		default: return "0x" +Integer.toHexString(msgid);
		}
	}

	public static String toProtocolString(byte[] pkt)
	{
		String str = "[" +MsgToName(pkt[2]) +"]";
		str+= " L:" +Byte.toString(pkt[1]);
		if(pkt[1] > 5) {
			str+= " O:" +Integer.toHexString(pkt[3] & 0xff);
		}
		if(pkt[1] > 6)
		{
			str+= " P:";
			for (int i = 4; i < (pkt[1] - 2); i ++) {
				str+= " " +Integer.toHexString(pkt[i] & 0xff);
			}
		}
