...
public class MetaWatchService extends Service {
	...
	void connect(Context context) {
		...
		try {
			...
			MetaWatch.logDebug("Remote device address: " + Preferences.watchMacAddress);
			...
			/*
			MetaWatch.logDebug("remote device name: " + bluetoothDevice.getName());
			...
			MetaWatch.logDebug("bond state: " + bond);
			 */
			...
			Log.e(MetaWatch.TAG, ioexception.toString());
			...
			public void run() {
				...
				while (run) {
					...
					case ConnectionState.DISCONNECTED:
						MetaWatch.logDebug("state: disconnected");
						break;
					case ConnectionState.CONNECTING:
						MetaWatch.logDebug("state: connecting");
					...
					case ConnectionState.CONNECTED:
						MetaWatch.logDebug("state: connected");
						// read from input stream
						readFromDevice();
						break;
					case ConnectionState.DISCONNECTING:
						MetaWatch.logDebug("state: disconnecting");
						// exit
						run = false;
						break;
					}
	...
	void readFromDevice() {
		...
		try {
			...
			MetaWatch.logDebug("started listening to watch...");
			int bc = inputStream.read(bytes);
			wakeLock.acquire(5000);

			MetaWatch.logDebug("received " +Protocol.toProtocolString(bytes));

			switch (bytes[2]) {
			case Protocol.Msg_GetDeviceTypeResponse:
				...
				break;
			case Protocol.Msg_NValResponse:
				/*if (bytes[3] == 0x00 && bytes[4] == 0x00) {
					//success && set to 12 hour format
					Protocol.setNvalTime(true);
				}*/
				break;
			case Protocol.Msg_StatusChangeEvent:
				if (bytes[4] == 0x11) {
					MetaWatch.logDebug("notify scroll request");
					synchronized (Notification.scrollRequest) {
						Notification.scrollRequest.notify();
					}
				}
				break;
			case Protocol.Msg_ButtonEventMessage:
				pressedButton(bytes[3]);
				break;
			}

		} catch (IOException e) {
			Log.e(MetaWatch.TAG, "stopped listening to watch... "+e);
			wakeLock.acquire(5000);
	...
	void pressedButton(byte button) {
		MetaWatch.logDebug("button code: " + Byte.toString(button));
		...
		case WatchStates.IDLE: {
			...
			/*
				if (Idle.isIdleButtonOverriden(button)) {
						MetaWatch.logDebug("this button is overriden");
